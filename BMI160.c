...
bmi160_raw_data_t bmi160_i2c_read_acc(void)
{
    ...
    x_data = raw_acc_data[0] | (int16_t) raw_acc_data[1]<<8;
    y_data = raw_acc_data[2] | (int16_t) raw_acc_data[3]<<8;
    z_data = raw_acc_data[4] | (int16_t) raw_acc_data[5]<<8;
    data_acc.x = x_data;
    data_acc.y = y_data;
    data_acc.z = z_data;
...
bmi160_raw_data_t bmi160_i2c_read_gyr(void) {
    ...
    x_data = raw_gyro_data[0] | (int16_t) raw_gyro_data[1]<<8;
    y_data = raw_gyro_data[2] | (int16_t) raw_gyro_data[3]<<8;
    z_data = raw_gyro_data[4] | (int16_t) raw_gyro_data[5]<<8;
    data_gyro.x = x_data;
    data_gyro.y = y_data;
    data_gyro.z = z_data;
